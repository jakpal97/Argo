...
const SERVICES = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="#60a5fa" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
function FormFooter({ items }) {
  return (
    <div className="flex items-center justify-center gap-5 px-8 py-3 border-t border-white/[0.06]" style={{ background: 'rgba(0,0,0,0.3)' }}>
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-1.5 text-[10px] font-medium text-slate-600 tracking-[0.04em] uppercase">
          {item.icon}
          {item.label}
        </div>
      ))}
...
function SuccessState({ title, subtitle, phone }) {
  return (
    <div className="flex flex-col items-center justify-center px-8 py-16 text-center gap-4 min-h-[380px]">
      ...
      <a href={`tel:${phone.replace(/\s/g, '')}`} className="text-base font-bold text-blue-400 hover:text-blue-300 transition-colors">
        {phone}
      </a>
    </div>
  )
...
function HeroForm() {
  ...
  const [form, setForm] = useState({
    ...
    objectType: '', objectSize: '', location: '', urgency: '', service: '', message: '',
    rodo: false,
  })
  ...
  return (
    <FormCard id="wycena">
      <FormHead
        ...
        title={<>Opisz swój obiekt —<br />dobierzemy rozwiązanie</>}
        subtitle="Każdy obiekt jest inny. Powiedz nam o swoim — przygotujemy ofertę skrojoną na miarę."
      />

      <div className="px-8 py-6 flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-3">
          <TextInput label="Imię i nazwisko *" id="f-name" value={form.name} onChange={set('name')} placeholder="Jan Kowalski" required />
          <TextInput label="Firma *" id="f-company" value={form.company} onChange={set('company')} placeholder="Nazwa firmy" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <TextInput label="E-mail służbowy *" id="f-email" type="email" value={form.email} onChange={set('email')} placeholder="jan@firma.pl" required />
          <TextInput label="Telefon *" id="f-phone" type="tel" value={form.phone} onChange={set('phone')} placeholder="+48 500 000 000" required />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <SelectInput
            label="Typ obiektu *" id="f-obj" value={form.objectType} onChange={set('objectType')} required
            options={[
          ...
          />
          <SelectInput
            label="Powierzchnia obiektu" id="f-size" value={form.objectSize} onChange={set('objectSize')}
            options={[
        ...
        <div className="grid grid-cols-2 gap-3">
          <TextInput label="Lokalizacja obiektu" id="f-loc" value={form.location} onChange={set('location')} placeholder="np. Gliwice, Katowice" />
          <SelectInput
            label="Pilność" id="f-urgency" value={form.urgency} onChange={set('urgency')}
            options={[
        ...
        <SelectInput
          label="Zakres usług" id="f-svc" value={form.service} onChange={set('service')}
          options={[
            { value: '', label: 'Co Cię interesuje?', disabled: true },
            { value: 'Stały FM outsourcing (kontrakt długoterminowy)', label: 'Stały FM outsourcing (kontrakt długoterminowy)' },
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-semibold text-slate-600 tracking-[0.07em] uppercase">Dodatkowe informacje</label>
          <textarea
            value={form.message}
        ...
        <div className="flex items-start gap-2.5 mt-1">
          ...
          <label htmlFor="f-rodo" className="text-[11px] text-slate-600 leading-relaxed cursor-pointer">
            Wyrażam zgodę na przetwarzanie danych osobowych przez ARGO System w celu przygotowania oferty. Dane szyfrowane SSL, nie są udostępniane osobom trzecim.
          </label>
        </div>
        ...
        >
          {isSubmitting ? 'Wysyłanie...' : <><span>Wyślij Zgłoszenie</span><IconSend /></>}
        </button>
      </div>

      <FormFooter items={[
        { label: 'DANE SZYFROWANE SSL', icon: <svg width="11" height="11" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></svg> },
        { label: 'RODO COMPLIANT', icon: <svg width="11" height="11" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg> },
        { label: 'ODDZWONIMY W 24H', icon: <svg width="11" height="11" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.5 10.5a19.79 19.79 0 01-3.07-8.67A2 2 0 012.42 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.91a16 16 0 006.29 6.29l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" /></svg> },
      ]} />
    </FormCard>
  )
}
...
function QuickForm() {
  ...
  return (
    <FormCard>
      ...
      />
      <div className="px-8 py-6 flex flex-col gap-3">
        <TextInput label="Imię i firma *" id="q-name" value={name} onChange={setName} placeholder="Jan Kowalski, Firma XYZ" required />
        <TextInput label="Telefon lub e-mail *" id="q-contact" value={contact} onChange={setContact} placeholder="+48 500 000 000 lub jan@firma.pl" required />
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-semibold text-slate-600 tracking-[0.07em] uppercase">Szczegóły zgłoszenia</label>
          <textarea
            value={message}
      ...
      <FormFooter items={[
        { label: 'ODPOWIEDŹ W 24H', icon: <svg width="11" height="11" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> },
        { label: 'DANE BEZPIECZNE', icon: <svg width="11" height="11" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></svg> },
      ]} />
    </FormCard>
  )
}
...
export default function WycenaPage() {
  ...
  return (
    <div className="bg-black text-white font-sans overflow-x-hidden" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  ...
                  <>
                    ...
                    <div key={s.label} className="flex flex-col gap-1">
                      <span className="font-extrabold text-[30px] text-white leading-none tracking-tight">
                        {s.val}<span className="text-slate-400 text-lg font-bold">{s.unit}</span>
                      </span>
                      <span className="text-[11px] font-medium text-slate-600 uppercase tracking-[0.08em]">{s.label}</span>
                    </div>
                  </>
                ))}
              </div>
      ...
      {/* ── TICKER ── */}
      <div className="border-t border-b border-white/[0.06] py-4 overflow-hidden" style={{ background: 'rgba(255,255,255,0.02)' }}>
      ...
      <section className="py-20 bg-black">
        <div className="max-w-[1220px] mx-auto px-10">
          ...
          <p className="text-[15px] text-slate-400 max-w-[500px] mb-14 leading-relaxed">
            Dostarczamy profesjonalne rozwiązania w zakresie maintenance i obsługi technicznej. Gwarantujemy bezpieczeństwo, niezawodność i pełną kontrolę.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {SERVICES.map(svc => (
              ...
              >
                ...
                <div className="w-11 h-11 rounded-xl flex items-center justify-center border border-blue-500/20 mb-5" style={{ background: 'rgba(59,124,246,0.12)' }}>
                  {svc.icon}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {svc.caps.map(c => (
                    <span key={c} className="px-2.5 py-1 rounded-full text-[11px] font-medium text-slate-600 border border-white/[0.08]" style={{ background: 'rgba(255,255,255,0.04)' }}>
                      {c}
                    </span>
                  ))}
                </div>
        ...
        <div className="max-w-[1220px] mx-auto px-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="relative rounded-2xl p-10 border border-blue-500/20 overflow-hidden" style={{ background: 'rgba(59,124,246,0.06)' }}>
              <div className="absolute top-0 right-0 w-56 h-56 rounded-full pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(59,124,246,0.15) 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />
              <div className="font-extrabold text-blue-400 leading-none tracking-tight mb-2" style={{ fontSize: '72px' }}>
                &lt;4<span className="text-slate-400 font-bold" style={{ fontSize: '30px' }}>h</span>
              </div>
              <div className="font-bold text-xl text-white mb-2.5">Gwarantowany czas reakcji</div>
              <p className="text-[13px] text-slate-400 leading-relaxed">
                Strategiczna lokalizacja w centrum aglomeracji śląskiej. Dotrzemy do każdego obiektu w województwie w czasie poniżej 4 godzin od zgłoszenia awarii.
              </p>
            </div>
            <div className="relative rounded-2xl p-10 border border-blue-500/20" style={{ background: 'rgba(59,124,246,0.06)' }}>
        ...
        <div className="max-w-[1220px] mx-auto px-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {WHY_ITEMS.map(item => (
              ...
              >
                <span className="text-[11px] font-bold text-blue-500 tracking-[0.08em] flex-shrink-0 pt-0.5">{item.n}</span>
                <div>
      ...
      <section className="py-20" style={{ background: '#0a0a0a' }}>
        <div className="max-w-[1220px] mx-auto px-10">
          ...
          >
            ...
            <h2 className="relative font-extrabold leading-tight tracking-tight mb-3.5" style={{ fontSize: 'clamp(28px, 3vw, 44px)' }}>
              Gotowy na współpracę<br />z <span className="text-blue-400">liderem branży?</span>
            </h2>
            <p className="relative text-[15px] text-slate-400 max-w-[500px] mx-auto mb-8 leading-relaxed">
              Skontaktuj się z nami. Razem przeanalizujemy potrzeby Twojego obiektu i dobierzemy model współpracy — bez sztucznych cenników.
            </p>
            ...
            <div>
              ...
              <div className="flex flex-col gap-3">
                {[
                  ...
                  { href: 'mailto:rdomagalski@argo-system.pl', icon: <IconMail />, label: 'Email kontaktowy', value: 'rdomagalski@argo-system.pl' },
                  { href: null, icon: <IconPin />, label: 'Obszar działania', value: 'Cała Polska · Baza: Śląsk' },
                ].map(item => (
                  item.href ? (
                    <a key={item.label} href={item.href} className="flex items-center gap-3.5 px-5 py-4 rounded-xl border border-white/[0.08] transition-all hover:border-blue-500/30 no-underline" style={{ background: 'rgba(255,255,255,0.04)' }}>
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center border border-blue-500/15 flex-shrink-0" style={{ background: 'rgba(59,124,246,0.1)' }}>
                        {item.icon}
                      </div>
                      <div>
                        <div className="text-[10px] font-semibold text-slate-600 tracking-[0.08em] uppercase mb-0.5">{item.label}</div>
                        <div className="text-[15px] font-semibold text-white">{item.value}</div>
                      </div>
                    </a>
                  ) : (
                    <div key={item.label} className="flex items-center gap-3.5 px-5 py-4 rounded-xl border border-white/[0.08]" style={{ background: 'rgba(255,255,255,0.04)' }}>
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center border border-blue-500/15 flex-shrink-0" style={{ background: 'rgba(59,124,246,0.1)' }}>
                        {item.icon}
                      </div>
                      <div>
      ...
      <footer className="flex items-center justify-between flex-wrap gap-4 px-10 py-6 border-t border-white/[0.07]">
        ...
        <div className="flex gap-6">
          <Link href="/polityka-prywatnosci" className="text-xs text-slate-600 hover:text-blue-400 transition-colors no-underline">Polityka Prywatności</Link>
          <Link href="/regulamin" className="text-xs text-slate-600 hover:text-blue-400 transition-colors no-underline">Regulamin</Link>
          <Link href="/" className="text-xs text-slate-600 hover:text-blue-400 transition-colors no-underline">Strona główna</Link>
        </div>
      </footer>
